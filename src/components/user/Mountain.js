...
const Mountain = () => {
  ...
  const fetchListings = async () => {
    try {
      const response = await axios.get('http://localhost:4000/Combine/allCombine?category=mountain');
  ...
  // Calculate total pages based on listings and listings per page
  const totalPages = Math.ceil(listings.length / listingsPerPage);

  // Slice listings based on current page and listings per page
            ...
            <div key={listing.combine_id} className="col-lg-3 col-md-6 mb-4">
              <div className="card mx-auto" style={{ maxWidth: '18rem' }}>
                ...
                <div className="card-body">
                  ...
                  <p className="card-text">
                    <small className="text-muted">To: {new Date(listing.endDate).toISOString().split('T')[0]}</small>
                  </p>
                  <p className="card-text">
  ...
  const renderPagination = () => {
    if (totalPages <= 1) {
      return null; // If there's only one page or no pages, do not render pagination
    }
